...
from .models import Site, Method, FeatureData, Variable
import requests
from requests_oauthlib import OAuth2Session
from oauthlib.oauth2 import BackendApplicationClient
from decouple import config
import datetime
from ews.helper_functions import predict_daterange
import pandas as pd
...
def contextBroker(contextBrokerID):
    ...
    token = oauth.fetch_token(token_url='https://k-rock.xyz/oauth2/token',
                              client_id=client_id,
                              client_secret=client_secret)
    headers = {'X-Auth-Token': token["access_token"]}
    get = requests.get(url="https://www.c-broker.xyz/v2/entities/"
                       + contextBrokerID + "/attrs/precipitation/value",
                       headers=headers,
                       verify=False)
...
def predict(model_id):
    model_id = 1
    predict = predict_daterange(start=(datetime
                                       .datetime
                                       .strptime(datetime
                                                 .datetime
                                                 .now(),
               ...
               'predictionValues': {'type': 'StructuredValue',
                                    'value': [
                                        ...
                                        {'percentile': '97.5',
                                            'value': quantiles[4]}],
                                    'metadata': {}}}
    patch = requests.patch(url=("https://www.c-broker.xyz/v2/entities/"
                                "urn:ngsi-ld:WaterQualityPredicted:"
                                "model1/attrs"),
                           headers={
                                'Content-Type': 'application/json',
                                'X-Auth-Token': KRock.KRock.get_token()},
...
def aggregate_to_daily(aggregation_date=datetime.date.today()):
    ...
    # aggregate rainfall
    rainfall = (pd.DataFrame(list(FeatureData.objects
                .filter(date__date=yesterday,
                        site__in=Site.objects
                        .filter(feature_type__name="Rainfall"))
    ...
    rainfall = (rainfall.groupby(["site_id",
                                  ...
                                  "method_id"])
                        .resample("D").sum()
                        .drop(["site_id", "variable_id", "method_id"], axis=1)
                        .reset_index())
    # aggregate data from other Feature Types
    others = (pd.DataFrame(list(FeatureData.objects
              .filter(date__date=yesterday,
                      site__in=Site.objects
                      .filter(feature_type__name__in=["WWTP",
                                                      "Riverflow",
                                                      "Network"]))
                                 .values())).set_index("date"))

    others = (others.groupby(["site_id",
                              "variable_id",
                              "method_id"])
                    .resample("D")
                    .mean()
