...
def import_data_from_model(model):
    ...
    for index, row in areas.iterrows():
        ...
        if type is None:
            logger.debug('feature_type is None in area {} of model with id {}'.format(index, model.id))
        site_ids = get_sites_of_type_in_geom(type, model.user, row['geom'])
        data = read_frame(FeatureData.objects.filter(site__in=site_ids), index_col='date')
        data['area'] = row['name']
...
def get_site_urls(model):
    ...
    ids = []
    for index, row in areas.iterrows():
        site_ids = get_sites_of_type_in_geom(row['feature_type'], model.user, row['geom'])
...
def get_sites_of_type_in_geom(type, owner, geom):
    logger.debug('Entering get_sites_of_type_in_geom({}, {}, geom)'.format(type, owner))
    df = read_frame(Site.objects.filter(
        feature_type=get_feature_type_named(type),
        owner=owner
    ))
    logger.debug('Site objects of type {} belonging to {}:'.format(type, owner))
...
def get_feature_type_named(name):
    ...
    if name is None:
        logger.debug('The name given to get_feature_type_named() is None. Returning None.')
        return None
...
def create_daily_lagvars(areavars, remove_duplicates=False):
    ...
    for i in range(len(areavars)):
        ...
        #areavars[i].value = pandas.to_numeric(areavars[i].value, downcast='float',  errors='coerce').interpolate()

        if remove_duplicates:
            areavars[i]= areavars[i][~areavars[i].index.duplicated(keep='first')]
        ...
        #df = pandas.DataFrame()
        #df[ft + '_shift_0'] = d.rolling(window=1).mean()
        #lagvars.append(areavars[i].value)
...
def predict_range(startdate, enddate, model, rawdata):
    ...
    # creating predictions using each individual 1000 trees (estimators) of the Random forest
    ...
    df = df.reset_index()
    #model = PredictionModel.objects.get(name = model.name)
...
def predict_daterange(start, end, model_id):
    ...
    areavars = import_data_from_model(model=PredictionModel.objects.get(name=model.name))
    ndata = create_daily_lagvars(areavars)
    predictions = predict_range(startdate=start, enddate=end, model = model, rawdata=ndata)
    return predictions
